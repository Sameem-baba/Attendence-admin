...
function AddAttendence() {
    ...
    const attendencemanage = async () => {
        if (preorab === 'Present') {
            ...
            await deleteDoc(doc(db, 'attendence', studentEmail)).then((user) => {alert('Student has been marked as absent')}).catch((err) => alert(err.message));;
        }
    }
        ...
        <div>
            <div className='bg-white rounded-3xl shadow-2xl flex w-full max-w-4xl'>
                    <div className='w-full text-center flex flex-col p-5'>
                        <div className='text-center font-bold cursor-pointer' onClick={() => router.push('/home')}>
                            <span className='text-green-500'>Company</span>Name
                        </div>
                        <div className='py-10'>
                            <h2 className='text-3xl font-bold text-green-500 mb-2 '>Attendence</h2>
                        ...
                        <div className='flex flex-col items-center '>
                            ...
                            <div className='mt-10'>
                                <button className='border-2 border-green-500 rounded-full px-12 py-2 inline-block font-semibold hover:bg-green-500 hover:text-white' type='submit' onClick={attendencemanage}>
                                    Add Attendence
                                </button>
                            </div>
