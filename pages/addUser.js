...
function AddUser() {
    ...
    const login = () => {
        createUserWithEmailAndPassword(auth, emailRef.current.value, passwordRef.current.value).then((data) => {
            ...
            <main className="flex flex-col items-center justify-center w-2/3 flex-1 text-center">
                <div className='bg-white rounded-3xl shadow-2xl flex w-2/3 max-w-4xl'>
                    <div className='w-3/5 text-center flex flex-col p-5'>
                        <div className='text-center font-bold' onClick={() => router.push('/')}>
                            <span className='text-green-500'>Company</span>Name
                        </div>
                        <div className='py-10'>
                            <h2 className='text-3xl font-bold text-green-500 mb-2 '>{isAdmins == true ? 'Add Teacher' : 'Add Student'}</h2>
